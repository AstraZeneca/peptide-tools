...
def calculateProteinCharge(pH):
    ...
    if NPhosphateGroups != 0:
        protein_charge += NPhosphateGroups * calculatePhosphateCharge(
            pH, pKa_noncanonical["pKa1_phosphate"], pKa_noncanonical["pKa2_phosphate"]
        )
    if NAlkylLysGroups != 0:
        protein_charge += NAlkylLysGroups * calculateBasicAminoAcidCharge(
            pH, pKa_basic["K"][0] + pKa_noncanonical["dpKa_alkylLys"]
        )
    if NDiAlkylLysGroups != 0:
        protein_charge += NDiAlkylLysGroups * calculateBasicAminoAcidCharge(
            pH, pKa_basic["K"][0] + pKa_noncanonical["dpKa_dialkylLys"]
        )
...
def split_sequence(sequence):
    ...
    if not lCyclic:
        ...
        if IonizableTerminiOfNTermRes == "_":
            IonizableTerminiOfNTermRes = sequence[0]
        else:
            if IonizableTerminiOfNTermRes == "":
                titAdd += ", capped N-terminus"
            else:
                titAdd += (
                    ", custom ionizable termini -NH2: " + IonizableTerminiOfNTermRes
                )

        if NTermRes == "_":
            NTermRes = sequence[0]
        else:
            titAdd += ", custom N termini residues: " + NTermRes

        if CTermRes == "_":
            CTermRes = sequence[-1]
        else:
            titAdd += ", custom C termini residues: " + CTermRes

        if IonizableTerminiOfCTermRes == "_":
            IonizableTerminiOfCTermRes = sequence[-1]
        else:
            if IonizableTerminiOfCTermRes == "":
                titAdd += ", capped C-terminus"
            else:
                titAdd += (
                    ", custom ionizable termini -COOH: " + IonizableTerminiOfCTermRes
                )
    ...
    if NPhosphateGroups > 0:
        titAdd += ", " + str(NPhosphateGroups) + " phosphorilated res"
    if NAlkylLysGroups > 0:
        titAdd += ", " + str(NAlkylLysGroups) + " monoalkyl Lys"
    if NDiAlkylLysGroups > 0:
        titAdd += ", " + str(NDiAlkylLysGroups) + " dialkyl Lys"
...
def print_pka_set():
    ...
    print(
        "For the following sets the pKa depends on the Residue poistion in the sequence, also the termini group pKa change among different terminal Residue."
    )
    ...
    #        print
    print(
        "ProMoST set: see http://isoelectric.ovh.org/theory.html and http://proteomics.mcw.edu/promost_adv.html  for details"
    )
    #        print
    ...
    #        print
    print(
        "Gauci_calibrated set: see Gauci et al. Proteomics 2008, 8, 4898  and  https://github.com/ypriverol/pIR   for details"
    )
    ...
    print()
    print(
        "   Alkylated Lys: addition to Lys pKa  "
        + str(pKa_noncanonical["dpKa_alkylLys"])
        + "      data from ACD lab: pKa of amine: 10.69. The delta for methylated amine compared to amine.  ### Zhang, Vogel, J. Bio. Chem. 1993, 268, 30, 22420 (Table III, Lys75) pKas of methylated 10.87, dimethylated 10.12"
    )
    print()
    print(
        "   Dialkylated Lys: addition to Lys pKa  "
        + str(pKa_noncanonical["dpKa_dialkylLys"])
...
def print_output(dict_pI_fasta, args):
    for molid_ind in dict_pI_fasta.keys():
        ...
        print_output_dict(
            dict_single["QpH7"], "Q at pH7.4", dict_single["plot_title_info"]
        )
        if args.lPrintpKa:
            print_pka_set()
    return
...
"""
    ...
    parser.add_option(
        ...
        action="store",
        dest="inputFile",
        help="input file name in fasta format",
        default="",
    )
    parser.add_option(
        "-g",
    ...
    parser.add_option(
        ...
        type="float",
        dest="tol",
        help="tolerance on total protein charge. default = 0.001",
        default=0.001,
    )
    ...
    parser.add_option(
        ...
        type="string",
        dest="CTermRes",
        help="Custom list of C terminus residues. By default it is set to the last residues of the given sequence. This option is useful if you have a branched peptide with several terminus residues",
        default="_",
    )
    parser.add_option(
        "-n",
        ...
        type="string",
        dest="NTermRes",
        help="Custom list of N terminus residues. By default it is set to the first residues of the given sequence. This option is useful if you have a branched peptide with several terminus residues",
        default="_",
    )
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        dest="IonizableTerminiOfCTermRes",
        help="Custom list of residues with ionizable C terminus (-COOH). By default it is set to the last residue of the given sequence. This option is useful if you want to cap the termini (exclude them from calculations) or you have a branched peptide with several terminus residues",
        default="_",
    )
    parser.add_option(
        "-b",
        action="store",
        type="string",
        dest="IonizableTerminiOfNTermRes",
        help="Custom list of residues with ionizable N terminus (-NH2). By default it is set to the first residue of the given sequence. This option is useful if you want to cap the termini (exclude them from calculations) or you have a branched peptide with several terminus residues",
        default="_",
    )
    ...
    parser.add_option(
        ...
        help="Number of dialkylated Lys residues. These residues should be denoted as X in the sequence. default = 0",
        default=0,
    )
    ...
    parser.add_option(
        ...
        type="string",
        dest="pka_set_list",
        help="List of pKa sets to use in calculation (comma separated). default = "
        + list_to_comma_seprated_string(pKa_sets_to_use),
        default="",
    )
    parser.add_option(
        "-r",
        action="store_true",
        dest="lCyclic",
        help="Is it cyclic? No termini residues are derived from sequence and also no ionizable termini (-NH2 and -COOH) of the terminal residues are derived from sequence (However, custom values are not overwritten). default = False",
        default=False,
    )
    parser.add_option(
        "-x",
        action="store_true",
        dest="lPlot",
        help="plot charge/pH curves. Requires NumPy and Matplotlib. default = False",
        default=False,
    )
    parser.add_option(
        "-q",
        action="store_true",
        dest="lIgnoreC",
        help="ignore cysteins, assume they are protected. default = False",
        default=False,
    )
    parser.add_option(
        "-z",
        action="store_true",
        dest="lPrintpKa",
        help="print used pKa values in the output. default = False",
        default=False,
    )
    parser.add_option(
        "-j",
        action="store_true",
        dest="l_json",
        help="use JSON as an output format. default = False",
        default=False,
    )
...
def plot_titration_curve(pH_Q_dict, figFileName):
    ...
    legend(
        loc="center right", bbox_to_anchor=[1.1, 0.5], ncol=1, shadow=True, fontsize=10
    ).get_frame().set_alpha(1)
...
def calc_pI_fasta(
    ...
    if len(args.seq) != 0:
        ...
        fasta = args.seq
        mol_supply_json = {}
        mol_supply_json[mol_unique_ind] = {
            "mol_name": mol_name,
            "mol_obj": None,
            "fasta": fasta,
        }
        ...
        raise Exception(
            "Error: either fasta, input file *.fasta or JSON should be given for pI_fasta.py. Exit. "
        )
        sys.exit(1)
...
def calc_pI_fasta_single_sequence(
    options={
        "seq": "",
        "tol": 0.001,
        "CTermRes": "_",
    ...
    if len(options["seq"]) == 0:
        raise Exception("no sequence given. See --help for more information")

    if options["pka_set_list"] != "":
        pKa_sets_to_use = options["pka_set_list"].split(",")

    for set in pKa_sets_to_use:
        if set not in all_known_pKa_sets:
            raise Exception(
                "---Error! pKa set "
                + set
                + " is not known. Check your -m option. Exit."
            )
    ...
    if len(seq) < 2:
        raise Exception(
            "---!Error: number of residues in sequence is less than 2. Not yet supported. Exiting."
        )
        sys.exit(1)
    ...
    for R in seq:
        if R not in known_res:
            raise Exception(
                "---!Error: residue "
                + R
                + " is not known. Please use X if this is a noncaninical residue. Exiting."
            )
            sys.exit(1)
    ...
    for R in NTermRes + MiddleSeq + CTermRes:
        if R in known_basic_res:
            nb += 1
        if R in known_acidic_res:
            na += 1
    ...
    if na == 0 and nb == 0:
        print(
            "---!Warning: no ionizable groups in the sequence. pI is not defined or = any pH. Exiting."
        )
        sys.exit(0)
    ...
    if na == 0 and nb != 0:
        print(
            "---!Warning: no acidic ionizable groups, only basic groups present in the sequence. pI is not defined and thus won't be calculated. However, will estimate pH when peptide has charge less than "
            + str(charge_tol * 100)
            + "% of the peptide maximum possible charge (by absolute value). Continue."
        )
        # lPlot = True
        # lCalc = True
    ...
    if nb == 0 and na != 0:
        print(
            "---!Warning: no basic ionizable groups, only acidic groups present in the sequence. pI is not defined and thus won't be calculated. However, will estimate pH when peptide has charge less than "
            + str(charge_tol * 100)
            + "% of the peptide maximum possible charge (by absolute value). Continue."
        )
        # lPlot = True
...
if __name__ == "__main__":
    ...
    if args.outputFile == "":  # output plain text
        ...
        if out_fext not in known_out_file_types:
            raise Exception(
                "Error! Output file extention not in supported file types:"
            ...
            with open(args.outputFile, "w") as csv_f:
                ...
                for mi in dict_out_pI_fasta.keys():
                    ...
                    row += [
                        " - ".join(
                            ["%.2f" % x for x in dict_out_pI_fasta[mi]["pI_interval"]]
                        )
                    ]
                    csv_w.writerow(row)
        ...
        dict_file = {
            ...
            + str(len(dict_out_pI_fasta.keys())),
        }
