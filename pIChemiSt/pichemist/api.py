...
def fasta_pkas_from_aa_list(aa_list,ionizable_nterm,ionizable_cterm):
    ...
    return FastaPKaMatcher().get_aa_pkas_from_aa_list(aa_list,ionizable_nterm,ionizable_cterm)
...
def smiles_to_base64(smiles):
    ...
    # Increase the font size for heteroatoms (e.g., N, O, S)
    #options.atomLabelFontSize = 20  # Set font size for atom labels

    # Minimize the empty space by adjusting the molecule's bounding box and padding
    img = Draw.MolToImage(mol, kekulize=True)
    #img = Draw.MolToImage(mol, kekulize=True, options=options)
