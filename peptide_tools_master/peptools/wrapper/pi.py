...
def _configure_options(mol_supply_json, runtime_params, chem_params):
    pI_fasta_options = {
        ...
        "lPrintpKa": False,
        "lPlot": runtime_params.generate_plots,
        "lIgnoreC": False,
...
def calculate_pichemist(mol_supply_json, run_params, io_params):
    dict_out_pIChemiSt = dict()
    if run_params.calc_pIChemiSt:
        plot_filename_prefix = "temp"
        if io_params.filepath_prefix:
        ...
        dict_out_pIChemiSt = pichemist_from_dict(
            ...
            plot_ph_q_curve=run_params.generate_plots,
            print_fragments=run_params.print_fragment_pkas,
        )
