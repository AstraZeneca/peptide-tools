    ...
    for R in known_res:
        if R not in pKa_TerminusIonizableGroup.keys():
            if R == "X":
                ...
                pKa_TerminusIonizableGroup[R] = [
                    (
                        pKa_TerminusIonizableGroup["E"][0]
                        + pKa_TerminusIonizableGroup["Q"][0]
                    )
                    / 2,
                    (
                        pKa_TerminusIonizableGroup["E"][1]
                        + pKa_TerminusIonizableGroup["Q"][1]
                    )
                    / 2,
                ]
            elif R == "B":
                pKa_TerminusIonizableGroup[R] = [
                    (
                        pKa_TerminusIonizableGroup["N"][0]
                        + pKa_TerminusIonizableGroup["D"][0]
                    )
                    / 2,
                    (
                        pKa_TerminusIonizableGroup["N"][1]
                        + pKa_TerminusIonizableGroup["D"][1]
                    )
                    / 2,
                ]
            elif R == "U":
                ...
                print(
                    "---!Error: data for specific -NH2 and -COOH termini pKa values for residue "
                    + R
                    + " is not given in the "
                    + SetName
                    + " pKa set. Set this residue identical to X (average of all available). Check set. Exit."
                )
                sys.exit(1)
...
pKa_acidic1 = {
    ...
    "U": [
        ...
        5.20,
        5.60,
    ],  # pK for U was taken from Byun et al. Biopolymers 2011, 95, 345
}


# Basic_Amino_Acids
#             AA    Primary  N-Terminal  C-Terminal
pKa_basic1 = {"R": [12.0, 12.0, 12.0], "H": [5.98, 5.98, 5.98], "K": [10.0, 10.0, 10.0]}
...
pKa_sets[SetName] = {
    "pKa_acidic": pKa_acidic1,
    "pKa_basic": pKa_basic1,
    "pKa_TerminusIonizableGroup": FillMissingAAtopKa_TerminusIonizableGroup(
        pKa_TerminusIonizableGroup1
    ),
}
